    ...
    function component(object) {
      ...
      this.find = function () {
        ...
        network.silent('http://api.themoviedb.org/3/' + (object.movie.name ? 'tv' : 'movie') + '/' + object.movie.id + '/external_ids?api_key=4ef0d7355d9ffb5151e987764708ce96&language=ru', function (ttid) {
          ...
          var url_end = Lampa.Utils.addUrlComponent(url, ttid.imdb_id ? 'imdb_id=' + encodeURIComponent(ttid.imdb_id) : 'title=' + encodeURIComponent(query));
          network.timeout(1000 * 15);
          network.silent(url_end, function (json) {
            if (json.data && json.data.length) display(json);else {
              network.silent(Lampa.Utils.addUrlComponent(url, 'title=' + encodeURIComponent(query)), display.bind(_this3), pillow.bind(_this3));
            }
          }, pillow.bind(this));
        }, function (a, c) {
          _this2.empty(network.errorDecode(a, c));
